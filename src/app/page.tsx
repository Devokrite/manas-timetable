...
const ALLOWED = new Set([1, 7, 10, 191]);
...
export default function SelectPage() {
  ...
  return (
    <main className="min-h-screen p-6 max-w-5xl mx-auto">
      ...
      <p className="mb-4 opacity-80">Showing 1. sınıf departments (4 selected).</p>
      <div className="mb-4 flex gap-2 items-center">
        <input className="rounded-xl border px-3 py-2 bg-white/10 flex-1"
          placeholder="Search department…" value={q} onChange={e => setQ(e.target.value)} />
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
        {filtered.map(d => (
          <Link key={d.id} href={`/departments/${d.id}`} className="rounded-2xl border hover:border-white/40 bg-white/5 p-3">
